...
class AdminController
{
	...
	public function createUser(): void
	{
		...
		if (!$email || strlen($password) < 6) {
			Response::json(['success'=>false,'error'=>['code'=>'VALIDATION','message'=>'Invalid email/password']], 422);
			return;
		}
		...
		if ($userRole === 'PATIENT') {
			...
			$stmt->execute([':id'=>$uid, ':doctor_id'=>$assignedDoctorId, ':address'=>$body['address'] ?? null]);
		}
		...
		if ($userRole === 'DOCTOR') {
			...
			$stmt->execute([':id'=>$uid, ':specialization'=>$body['specialization'] ?? null]);
		}
