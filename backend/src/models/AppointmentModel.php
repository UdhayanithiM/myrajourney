...
class AppointmentModel
{
	...
	{
		$w = [];$p = [];
		if (!empty($filters['patient_id'])) { $w[]='a.patient_id = :pid'; $p[':pid']=(int)$filters['patient_id']; }
		if (!empty($filters['doctor_id'])) { $w[]='a.doctor_id = :did'; $p[':did']=(int)$filters['doctor_id']; }
		$where = $w ? ('WHERE '.implode(' AND ',$w)) : '';
		$offset = ($page-1)*$limit;
		$total = (int)$this->db->query("SELECT COUNT(*) FROM appointments a $where" )->fetchColumn();
